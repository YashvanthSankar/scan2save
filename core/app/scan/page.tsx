    ...
    try {
      ...
      if (videoRef.current) {
        ...
        try {
          ...
          // Handle AbortError gracefully (happens when user navigates away or component unmounts)
          if (playErr.name === 'AbortError') {
            console.log('Camera playback aborted - likely navigation');
            return;
          }
          throw playErr;
        }
        ...
        <div className="flex flex-col items-center gap-4 max-w-md mx-auto">
          ...
          <div className="flex w-full gap-3">
            ...
            <button
              ...
              className={`flex-1 py-4 rounded-2xl flex items-center justify-center gap-2 font-bold transition-all ${cameraActive
          ...
          {/* Development Only: Simulate Scan */}
          <button
            onClick={() => handleScanSuccess(JSON.stringify({
              app: 'scan2save',
              sid: 'freshmart-blr-01',
              name: 'FreshMart',
              location: 'Indiranagar',
              timestamp: Date.now()
            }))}
            className="text-xs text-gray-600 underline hover:text-green-500 mt-2"
          >
            [DEV] Simulate Scan (FreshMart)
          </button>
        </div>
      </div>
    </div>
