...
export default function ManageStores({ initialStores }: ManageStoresProps) {
    ...
    const handleDelete = async (id: string) => {
        ...
        try {
            await fetch(`/api/stores?id=${id}`, { method: 'DELETE' }); // Assuming delete endpoint exists or similar
            showNotification('success', 'Store removed');
        } catch (error) {
            setStores(previousStores);
            showNotification('error', 'Failed to remove store');
        }
    };
