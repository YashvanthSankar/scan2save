...
export default function ManageStores() {
  ...
  const fetchStores = async () => {
    try {
      // In real app: const res = await fetch('/api/stores');
      // For now, we simulate a delay with mock data
      setTimeout(() => {
        setStores([
          { id: '1', storeId: 'RELIANCE_MUM_01', name: 'Reliance Smart', location: 'Bandra West, Mumbai', lat: 19.0, lng: 72.8 },
          { id: '2', storeId: 'DMART_BLR_01', name: 'DMart HSR', location: 'HSR Layout, Bangalore', lat: 12.9, lng: 77.6 },
        ]);
        setLoading(false);
      }, 1000);
    } catch (e) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    // In real app: POST to /api/stores
    const newStore = {
      id: Math.random().toString(),
      storeId: formData.storeId.toUpperCase(),
      name: formData.name,
